...
public class LoadingScreenBarSystem : MonoBehaviour
{
    ...
    private void Awake()
    {
        this._btnExit = this.transform.Find("Btn_Exit").GetComponent<Button>();
    }

    private void OnEnable()
    {
        Launcher.Instance.SetBtnEvent(this._btnExit, () => { Destroy(this.gameObject); });
    }
    ...
    */
    ...
    {
        yield return null;
        // var handle = Addressables.LoadSceneAsync("Assets/Res/RPGPP_LT/Scene/rpgpp_lt_scene_1.0.unity", LoadSceneMode.Single, true);
        // handle.Completed += (obj) => { Debug.LogWarning($"Load async scene complete{obj.Status}"); };
        // while (!handle.IsDone)
        // {
